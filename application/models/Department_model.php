...
class Department_model extends CI_Model {
    ...
    {
        $query = $this->db->query("SELECT DISTINCT item_detail.item_id,distribution.dist_id as dist_id, department, item_name, item_description, distribution.quantity as quantity, unit FROM department LEFT JOIN distribution ON department.dept_id = distribution.dept_id LEFT JOIN item_detail ON distribution.dist_id = item_detail.dist_id LEFT JOIN item ON item.item_id = item_detail.item_id WHERE item_detail.dist_id IS NOT NULL AND distribution.dept_id = $id");
        return $query->result_array();
    }
